 ...
 * @description Storage Paths Test
 */

import { browserBindingFile } from "../../../../src/main/browser/browser-binding-file";
import { parseAppConfig } from "../../../../src/main/storage/app-config";
import { loadStoragePaths, resolveStoragePaths } from "../../../../src/main/storage/storage-paths";
...
describe("desktop storage paths", () => {
    ...
    it("preserves existing workspace data when no configuration exists", async () => {
        ...
        expect(paths.agentRuntime).toBe(join(directory, "agent-runtime"));
    ...
    it.each([
        ...
        { dataDirectory: null },
        { dataDirectory: 1 },
        { dataDirectory: "/bad\0path" },
        { dataDiretory: "/misspelled" },
    ])("rejects invalid configuration without falling back: %j", (configuration) => {

        expect(() => {

            return parseAppConfig(configuration);
        }).toThrow();
    });
