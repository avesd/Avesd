 ...
 * @author Avesd
 * @package Desktop
 * @namespace TestUnitRendererSrcWorkbench
 * @description Web Results Test
 */

import { WebResults } from "../../../../../src/renderer/src/workbench/web-results";
import { WEB_PLUGIN_ID, WEB_RESULT_TYPE } from "../../../../../src/shared/browser/web-surface";
...
describe("temporary web outputs", () => {
    ...
    it("binds a live output without saving content and rejects writes or cross-dashboard binding", async () => {
        ...
        await repository.createDashboard(scope, {
            id: scope.dashboardId,
            name: "Dashboard",
            viewState: {},
        });
        await repository.createDashboard(scope, {
            id: "other" as DashboardId,
            name: "Other",
            viewState: {},
        });
        ...
        const data = results.wrap(new WorkspaceDataCoordinator(repository, () => {
        ...
        await layouts.apply(scope, {
            ...
            operations: [
                {
                    type: "add",
                    id: "page" as WidgetInstanceId,
                    pluginId: WEB_PLUGIN_ID,
                    widgetTypeId: "page",
                },
                {
                    type: "add",
                    id: "consumer" as WidgetInstanceId,
                    pluginId: WEB_PLUGIN_ID,
                    widgetTypeId: "result",
                },
            ],
        ...
        const other = {
            ...scope,
            dashboardId: "other" as DashboardId,
        };
        expect(await data.list(other)).toEqual([]);
        await expect(data.read(other, id)).rejects.toThrow("incompatible");
        await layouts.apply(other, {
            expectedRevision: 0,
            operations: [
                {
                    type: "add",
                    id: "other-consumer" as WidgetInstanceId,
                    pluginId: WEB_PLUGIN_ID,
                    widgetTypeId: "result",
                },
            ],
        });
        await expect(layouts.apply(other, {
            expectedRevision: 1,
            operations: [
                {
                    type: "bind",
