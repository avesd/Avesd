 ...
 * @description Widget Services Test
 */

import { createWidgetServices } from "../../../../../src/renderer/src/workbench/widget-services";
import type { BrowserControlsApi } from "../../../../../src/shared/browser/browser-controls";
import { WEB_PLUGIN_ID } from "../../../../../src/shared/browser/web-surface";
...
it("grants only built-in controllers a browser service bound to their own instance", async () => {
    ...
    const factory = createWidgetServices(new WorkspaceDataCoordinator(new InMemoryWorkspaceRepository(), () => {

        return undefined;
    }), api);
    const instance = {
        id: "controller" as WidgetInstanceId,
    ...
    expect(invoke).toHaveBeenCalledWith("controller", "browser", {
        type: "extract",
        fields: { text: "h1" },
    });
