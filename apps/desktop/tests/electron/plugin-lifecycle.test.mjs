...
import { createDesktopHarness, desktop, expect } from "./support/desktop.mjs";
import assert from "node:assert/strict";
import { spawn } from "node:child_process";
import { join } from "node:path";
import { createInterface } from "node:readline";

const harness = await createDesktopHarness("plugin-lifecycle");
const relayPath = join(desktop, "out/main/workspace-mcp.js");
let app;
let relay;
let sequence = 0;
...
const launch = async () => {
    ...
    const gateway = await app.evaluate(() => {
        return globalThis.__avesdTestGateway;
    });
    relay = spawn(process.execPath, [relayPath], {
        env: {
            ...process.env,
            AVESD_HOST_URL: gateway.url,
            AVESD_HOST_TOKEN: gateway.token,
        },
        stdio: [
            "pipe",
            "pipe",
    ...
    createInterface({ input: relay.stdout }).on("line", line=>{
        ...
        const waiter=pending.get(message.id); if (!waiter) {
            return;
        }
    ...
    await request("initialize", {
        ...
        capabilities: {},
        clientInfo: {
            name: "synthetic-authoring-harness",
            version: "1.0.0",
        },
    });
...
const request = (method, params) => {
    return new Promise((resolve, reject)=>{
        ...
        const timer=setTimeout(()=>{
            pending.delete(id);reject(new Error(`MCP request timed out: ${method}`));
        }, 20000);
        pending.set(id, {
            resolve,
            reject,
...
const tool=async(name, args={})=>{
    ...
    assert.ok(!response.isError, response.content?.find(item=>{
        return item.type==="text";
    })?.text);
    return JSON.parse(response.content.find(item=>{
        return item.type==="text";
    }).text);
};
const stopRelay=()=>{
    relay?.kill();relay=undefined;
};
try {
    ...
    const content={
        ...sdk.example,
        manifest: {
            ...sdk.example.manifest,
            id: "avesd.local.focus-counter",
    ...
    assert.equal((await rawTool("avesd_activate_plugin", {
        draftId: draft.id,
        revision: draft.revision,
    })).isError, true);
    draft=await tool("avesd_write_plugin_draft", {
        ...content,
        source: content.source.replace("String(++count)", "String(count)"),
        draftId: draft.id,
        expectedRevision: draft.revision,
    });
    const failed=await tool("avesd_test_plugin", {
        draftId: draft.id,
        revision: draft.revision,
    });
    ...
    const tested=await rawTool("avesd_test_plugin", {
        draftId: draft.id,
        revision: draft.revision,
    });
    assert.ok(!tested.isError);
    const report=JSON.parse(tested.content.find(item=>{
        return item.type==="text";
    }).text);
    assert.equal(report.passed, true, JSON.stringify(report));
    console.log("Local authoring: interaction failure, repair, and passing report verified");
    assert.ok(tested.content.some(item=>{
    ...
    const preview=await rawTool("avesd_preview_widget", {
        draftId: draft.id,
        revision: draft.revision,
    });
    assert.ok(!preview.isError);
    assert.ok(preview.content.some(item => {
        return item.type === "image";
    }));
    await app.evaluate(({ BrowserWindow }) => {
        BrowserWindow.getAllWindows().find(window=>{
            return window.getTitle().endsWith("· Preview");
        })
            ?.destroy();
    });
    assert.ok(!(await rawTool("avesd_preview_widget", {
        draftId: draft.id,
        revision: draft.revision,
    })).isError);
    ...
    assert.ok((await tool("avesd_inspect_dashboard")).availableWidgetTypes.some(widget=>{
        return widget.pluginId===content.manifest.id;
    }));
    await expect.poll(()=>{
        return page.evaluate(()=>{
            return window.avesd.localPlugins.list();
        });
    }).toHaveLength(1);
    const added=await tool("avesd_add_widget", {
    ...
    await expect.poll(()=>{
        return page.evaluate(async()=> {
            return (await window.avesd.workspaceStorage.load()).widgets.length;
        });
    }).toBe(1);
    const guest=async()=>{
        return app.evaluate(({ BrowserWindow })=>{
            ...
            const view=host.contentView.children.find(view=>{
                return view.webContents?.getURL().startsWith("data:");
            });
    ...
    await app.evaluate(async({ webContents }, id)=>{
        return webContents.fromId(id).executeJavaScript("document.querySelector('#widget').shadowRoot.querySelector('button').click()");
    }, id);
    assert.equal(await app.evaluate(async({ webContents }, id)=>{
        return webContents.fromId(id).executeJavaScript("document.querySelector('#widget').shadowRoot.querySelector('output').textContent");
    }, id), "1");
    await page.keyboard.press("Meta+e");
    await page.getByText("Editing layout", { exact: true }).waitFor();
    ...
    stopRelay();await harness.close();app=undefined;
    page=await launch();
    await expect.poll(guest).not.toBeNull();
    assert.ok((await tool("avesd_inspect_dashboard")).availableWidgetTypes.some(widget=>{
        return widget.pluginId===content.manifest.id;
    ...
    console.log(JSON.stringify({
        ...
        checks: [
            "real MCP relay",
            "draft read/write",
            "failed interaction rejected",
            "stale revision rejected",
            "preview image",
            "live catalog",
            "live dashboard",
            "isolated native widget",
            "interaction",
            "overlay visibility",
            "restart persistence",
            "remove",
