...
const createStore = async () => {
  ...
  return { directory, store: new LocalPluginStore(directory) };
};
...
describe("local plugin installation", () => {
  it("requires a passing exact revision, invalidates edited reports, and reopens installed code", async () => {
    ...
    store.record({ draftId: draft.id, revision: draft.revision, passed: false, checks: [] });
    await expect(store.activate(draft.id, draft.revision)).rejects.toThrow("passing tests");
    store.record({ draftId: draft.id, revision: draft.revision, passed: true, checks: [] });
    await store.activate(draft.id, draft.revision);
    const edited = await store.write(draft.id, draft.revision, { ...counterExample, source: `${counterExample.source}\n// Edited` });
  ...
  it("rejects path traversal, builtin identities, oversized source, and empty tests", async () => {
    ...
    await expect(store.create({ ...counterExample, manifest: { ...counterExample.manifest, id: "avesd.builtin.counter" } })).rejects.toThrow();
    await expect(store.create({ ...counterExample, source: "x".repeat(65_537) })).rejects.toThrow();
    await expect(store.create({ ...counterExample, tests: [] })).rejects.toThrow();
  });
});
