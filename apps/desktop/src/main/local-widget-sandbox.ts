...
import type { WidgetWorkspaceBridge } from "./widget-workspace-bridge";
import { widgetWorkspaceChannels } from "../shared/widget-workspace";
import { randomUUID } from "node:crypto";
import { BrowserWindow, session } from "electron";
import type { WebContents, WebPreferences } from "electron";
import type { LocalPluginDraft, PluginTestReport, WidgetTestStep } from "../shared/local-plugins";
...
export function localWidgetPreferences(): WebPreferences {
  ...
  isolatedSession.webRequest.onBeforeRequest((details, callback) =>
    callback({ cancel: !details.url.startsWith("data:") && !details.url.startsWith("blob:") }));
  return { preload: join(__dirname, "../preload/local-widget.cjs"), session: isolatedSession, sandbox: true, contextIsolation: true, nodeIntegration: false,
    nodeIntegrationInSubFrames: false, webSecurity: true, allowRunningInsecureContent: false,
    navigateOnDragDrop: false, safeDialogs: true, disableDialogs: true, webviewTag: false };
}

const literal = (input: unknown) => JSON.stringify(input).replaceAll("<", "\\u003c");
...
export async function bounded<T>(work: Promise<T>, milliseconds: number, timeout: () => void = () => {}): Promise<T> {
  let timer: ReturnType<typeof setTimeout> | undefined;
  try {
    return await Promise.race([work, new Promise<never>((_resolve, reject) => {
      timer = setTimeout(() => { timeout(); reject(new Error("Widget operation timed out.")); }, milliseconds);
    })]);
...
export class LocalWidgetRunner {
  ...
  async preview(draft: LocalPluginDraft): Promise<string> {
    if (this.#preview && !this.#preview.isDestroyed()) this.#preview.destroy();
    const window = this.#create(draft);
    this.#preview = window;
    try {
      await bounded(loadLocalWidget(window.webContents, draft), 5000, () => window.destroy());
      window.show();
  ...
  async test(draft: LocalPluginDraft): Promise<{ report: PluginTestReport; image?: string }> {
    const window = this.#create(draft);
    const checks: { name: string; passed: boolean; message?: string }[] = [];
    let image: string | undefined;
    const run = async () => {
      await loadLocalWidget(window.webContents, draft);
      checks.push({ name: "Module, mount, and initial update", passed: true });
      const isolation = await window.webContents.executeJavaScript(
        "typeof require === 'undefined' && typeof process === 'undefined' && typeof window.avesd === 'undefined' && location.origin === 'null'",
      ) as boolean;
      if (!isolation) throw new Error("Widget environment isolation check failed.");
      checks.push({ name: "No Node or host API", passed: true });
      const rendered = await this.#evaluate(window.webContents,
        "const root = document.querySelector('#widget')?.shadowRoot; return !!root?.textContent?.trim() && root.children.length > 0;");
      if (!rendered) throw new Error("Widget rendered no content.");
      checks.push({ name: "Rendered content", passed: true });
      for (const [index, step] of draft.tests.entries()) {
        await this.#step(window.webContents, step);
        checks.push({ name: `Interaction ${index + 1}: ${step.type}`, passed: true });
      }
      ...
      if (errors) throw new Error("Widget raised a runtime error or attempted a blocked capability.");
      checks.push({ name: "Update without runtime errors", passed: true });
      await window.webContents.executeJavaScript("window.__avesdWidget.dispose()");
      checks.push({ name: "Abort and dispose", passed: true });
    };
    try { await bounded(run(), 10_000, () => window.destroy()); }
    catch (error) {
      checks.push({ name: "Widget execution", passed: false,
    ...
    return { report: { draftId: draft.id, revision: draft.revision, passed: checks.every((check) => check.passed), checks }, image };
  }

  #create(draft: LocalPluginDraft): BrowserWindow {
    const window = new BrowserWindow({ show: false, width: draft.manifest.size.width * 40,
      height: draft.manifest.size.height * 24, useContentSize: true, title: `${draft.manifest.displayName} · Preview`,
      webPreferences: { ...localWidgetPreferences(), backgroundThrottling: false } });
    const workspaceId = "preview-workspace" as WorkspaceId;
    const dashboardId = "preview-dashboard" as DashboardId;
    let snapshot: WorkspaceSnapshot = {
      version: 1, selection: { workspaceId, dashboardId },
      workspaces: [{ id: workspaceId, name: "Preview workspace" }],
      dashboards: [{ id: dashboardId, workspaceId, name: "Preview dashboard", viewState: {}, layoutRevision: 0 }],
      widgets: [], dataSources: [],
    };
    let queue: Promise<unknown> = Promise.resolve();
    const disposeServices = this.bridge.register(window.webContents, {
      identity: { workspaceId, dashboardId, instanceId: "preview-widget" as WidgetInstanceId, capabilities: draft.manifest.capabilities ?? [] },
      invoke: (request, isActive) => {
        const work = queue.then(async () => {
          if (!isActive()) throw new Error("Preview is closed.");
  ...
  async #step(contents: WebContents, step: WidgetTestStep): Promise<void> {
    ...
    if (step.type === "click") {
      ...
      while (true) {
        ...
        if (matches) return;
        if (Date.now() >= deadline) throw new Error(`Text assertion failed for ${step.selector}.`);
        await new Promise((resolve) => setTimeout(resolve, 25));
      }
    }
  }

  dispose(): void { for (const window of this.#windows) if (!window.isDestroyed()) window.destroy(); }
}
