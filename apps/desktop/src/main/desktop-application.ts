...
export function startDesktop() {
    ...
    let agentHost: AcpAgentHost | undefined;
    ...
    const resetWindowServices = (window: BrowserWindow): void => {
        ...
        localWidgetSurfaces = new LocalWidgetSurfaces(window, workbench.plugins, () => {
            return workspaceFile!.load();
        }, widgetWorkspaceBridge, workbench);
        if (!agentRuntimeRoot) {
            throw new Error("Agent runtime storage is not ready");
        }
        agentHost = new AcpAgentHost(agentRuntimeRoot, gateway, join(__dirname, "workspace-mcp.js"), agentPreferences, createAgentProviders(providerInstallations), () => {
            return workbench!.agentContext();
        });
    ...
    const hostForEvent = (event: IpcMainInvokeEvent): AcpAgentHost => {
    ...
    ipcMain.handle(workbenchPreferencesChannels.get, event => {
        hostForEvent(event); return workbenchPreferences.side;
    });
    ipcMain.handle(workbenchPreferencesChannels.set, (event, side: unknown) => {
        hostForEvent(event); return workbenchPreferences.setSide(side);
    });
