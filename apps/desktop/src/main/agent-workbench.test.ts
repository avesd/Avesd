...
import { counterExample } from "./local-plugin-contract";
import type { AgentToolResult } from "./agent-tools";
import type { LocalPluginDraft } from "../shared/local-plugins";

const directories: string[] = [];
afterEach(async () => { await Promise.all(directories.splice(0).map((directory) => rm(directory, { recursive: true, force: true }))); });
const textResult = (result: AgentToolResult): unknown => JSON.parse(result.content.find((item) => item.type === "text")!.text);
...
it("exposes installed types immediately and protects agent writes against stale renderer saves", async () => {
  ...
  const scope = { workspaceId: "test-workspace" as WorkspaceId, dashboardId: "test-dashboard" as DashboardId };
  await file.save({ version: 1, workspaces: [{ id: scope.workspaceId, name: "Test" }],
    dashboards: [{ id: scope.dashboardId, workspaceId: scope.workspaceId, name: "Test", layoutRevision: 0, viewState: {} }], widgets: [], dataSources: [] });
  const changed = vi.fn();
  const pluginsChanged = vi.fn();
  const workbench = new AgentWorkbench(file, new LocalPluginStore(join(directory, "plugins")), {
    async test(draft) { return { report: { draftId: draft.id, revision: draft.revision, passed: true, checks: [] } }; },
    async preview() { return "synthetic-image"; },
  }, changed, pluginsChanged);
  workbench.configure({ widgetDefinitions: [], dataSourceDefinitions: [] });
  ...
  await workbench.invoke("avesd_test_plugin", { draftId: draft.id, revision: draft.revision });
  await workbench.invoke("avesd_activate_plugin", { draftId: draft.id, revision: draft.revision });
  expect(pluginsChanged).toHaveBeenCalledOnce();
  const inspection = textResult(await workbench.invoke("avesd_inspect_dashboard", {})) as { availableWidgetTypes: { pluginId: string }[] };
  expect(inspection.availableWidgetTypes[0]?.pluginId).toBe(counterExample.manifest.id);
  const before = await file.load();
  await Promise.all([1, 2].map(() => workbench.invoke("avesd_add_widget", { pluginId: counterExample.manifest.id, widgetTypeId: "counter" })));
...
it.each(["avesd_test_plugin", "avesd_preview_widget"] as const)(
  ...
  async (tool) => {
    ...
    const file = new WorkspaceFile(join(directory, "workspace.json"));
    const initial = { version: 1 as const, workspaces: [], dashboards: [], widgets: [], dataSources: [] };
    ...
    const workbench = new AgentWorkbench(file, plugins, {
      async test(draft) {
        ...
        return { report: { draftId: draft.id, revision: draft.revision, passed: true, checks: [] } };
      },
      async preview() { started.resolve(); await finish.promise; return "synthetic-image"; },
    }, changed, vi.fn());
    const draft = await plugins.create(counterExample);
    const execution = workbench.invoke(tool, { draftId: draft.id, revision: draft.revision });
    const outcome = execution.catch((error: unknown) => error);
    await started.promise;
    try {
      let saved = false;
      const save = workbench.save(initial, { snapshot: initial }).then(() => { saved = true; });
      await vi.waitFor(() => expect(saved).toBe(true));
      await save;
      expect(changed).toHaveBeenCalledOnce();
      let edited = false;
      const write = workbench.invoke("avesd_write_plugin_draft", {
        ...counterExample, source: `${counterExample.source}\n// Revised during execution.`,
    ...
    await expect(workbench.invoke("avesd_activate_plugin", {
      draftId: draft.id, revision: current.revision,
    })).rejects.toThrow("passing tests");
    await workbench.invoke("avesd_test_plugin", { draftId: draft.id, revision: current.revision });
    await expect(workbench.invoke("avesd_activate_plugin", {
      draftId: draft.id, revision: current.revision,
...
it("serializes native runner jobs and recovers after a failed execution", async () => {
  ...
  const started = gate();
  const finish = gate();
  const preview = vi.fn(async () => "synthetic-image");
  const workbench = new AgentWorkbench(new WorkspaceFile(join(directory, "workspace.json")), plugins, {
    async test() { started.resolve(); await finish.promise; throw new Error("Runner failed"); },
    preview,
  }, vi.fn(), vi.fn());
  const draft = await plugins.create(counterExample);
  const command = { draftId: draft.id, revision: draft.revision };
  const test = workbench.invoke("avesd_test_plugin", command).catch((error: unknown) => error);
  await started.promise;
  const next = workbench.invoke("avesd_preview_widget", command);
  await workbench.save({ version: 1, workspaces: [], dashboards: [], widgets: [], dataSources: [] });
...
it("commits selection with dashboards, rejects queued old-scope tools, and preserves state on save failure", async () => {
  ...
  workbench.configure({ dataSourceDefinitions: [], widgetDefinitions: [] });
  const first = await workbench.navigate({ type: "inspect" });
  const original = await file.load();
  const creating = workbench.navigate({ type: "create", workspaceId: first.scope.workspaceId, name: "Focus" });
  const staleTool = workbench.invoke("avesd_inspect_dashboard", {}).catch((error: unknown) => error);
  const second = await creating;
  expect((await staleTool as Error).message).toContain("dashboard changed");
  const inspection = textResult(await workbench.invoke("avesd_inspect_dashboard", {})) as { layout: { dashboardId: string } };
  ...
  await expect(workbench.navigate({ type: "select", scope: first.scope })).rejects.toThrow("disk failure");
  save.mockRestore();
...
it("authorizes widget capabilities inside the transaction queue and rejects disposed or stale callers", async () => {
  ...
  workbench.configure({ widgetDefinitions: [], dataSourceDefinitions: [] });
  const first = await workbench.navigate({ type: "inspect" });
  const content = { ...counterExample, manifest: { ...counterExample.manifest, capabilities: ["catalog" as const] } };
  const draft = await store.create(content);
  store.record({ draftId: draft.id, revision: draft.revision, passed: true, checks: [] });
  await store.activate(draft.id, draft.revision);
  await workbench.invoke("avesd_add_widget", { pluginId: draft.manifest.id, widgetTypeId: draft.manifest.widgetTypeId });
  ...
  await expect(workbench.invokeWidget(widgetId, { type: "command", command: { type: "deleteWorkspace", workspaceId: first.scope.workspaceId } })).rejects.toThrow("not granted");
  await expect(workbench.invokeWidget(widgetId, { type: "workspaces" }, () => false)).rejects.toThrow("no longer active");
  const switching = workbench.navigate({ type: "createWorkspace", name: "Second" });
  ...
  await rejected;
  await workbench.navigate({ type: "select", scope: first.scope });
  await expect(workbench.invokeWidget(widgetId, { type: "dashboards", workspaceId: second.scope.workspaceId }))
    .resolves.toEqual(second.dashboards.map(({ id, name, workspaceId }) => ({ id, name, workspaceId })));
  const before = await file.load();
  await expect(workbench.invokeWidget(widgetId, { type: "command", command: { type: "select", scope: second.scope } })).rejects.toThrow("not granted");
