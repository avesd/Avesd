...
const server = createAgentMcpServer(async (name, input) => {
  ...
  }, body: JSON.stringify({ name, input }), signal: AbortSignal.timeout(30_000) });
  if (!response.ok) throw new Error("Avesd host is unavailable.");
  return await response.json() as AgentToolResult;
});

serveStdio(() => server, { onerror() { process.stderr.write("Avesd MCP request failed.\n"); } });
