...
import type { LocalPluginDraft, LocalPluginSummary, PluginTestReport } from "../shared/local-plugins";
...
const revisionOf = (content: DraftContent) => createHash("sha256").update(JSON.stringify(content)).digest("hex");
...
export class LocalPluginStore {
  ...
  async create(input: unknown): Promise<LocalPluginDraft> {
    ...
    const draft = { ...content, id: randomUUID(), revision: revisionOf(content) };
  ...
  async read(id: string): Promise<LocalPluginDraft> {
    ...
    const content = draftContentSchema.parse(stored && typeof stored === "object"
      ? { manifest: Reflect.get(stored, "manifest"), source: Reflect.get(stored, "source"), tests: Reflect.get(stored, "tests") } : stored);
    return { ...content, id, revision: revisionOf(content) };
  }

  async write(id: string, expectedRevision: string, input: unknown): Promise<LocalPluginDraft> {
    const previous = await this.read(id);
    if (previous.revision !== revisionSchema.parse(expectedRevision)) throw new Error("Draft changed; read it before editing.");
    const content = draftContentSchema.parse(input);
    if (content.manifest.id !== previous.manifest.id) throw new Error("A draft's plugin identity cannot change.");
    const draft = { ...content, id, revision: revisionOf(content) };
  ...
  async activate(id: string, revision: string): Promise<LocalPluginSummary> {
    ...
    const previous = installed.find((plugin) => plugin.manifest.id === draft.manifest.id);
    ...
    return { manifest: draft.manifest, revision: draft.revision };
  }
  ...
  async installed(id: string): Promise<LocalPluginDraft> {
    ...
    const content = draftContentSchema.parse({ manifest: stored.manifest, source: stored.source, tests: stored.tests });
    if (content.manifest.id !== id || revisionOf(content) !== stored.revision) throw new Error("Installed plugin integrity check failed.");
    return { ...content, id: draftIdSchema.parse(stored.id), revision: stored.revision };
  }
  ...
  async list(): Promise<readonly LocalPluginSummary[]> {
    ...
    return Promise.all(files.filter((file) => /^avesd\.local\.[a-z][a-z0-9-]{0,63}\.json$/.test(file)).sort().map(async (file) => {
