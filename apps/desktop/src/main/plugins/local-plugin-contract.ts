...
export const localPluginSdk = {
    ...
    lifecycle: "The host calls mount, then update with empty configuration and the current instance grid size, including resize previews. Manifest size is the initial size only; users can resize each instance. Unstored state is in memory. Private files and SQLite persist per workspace and plugin. Destroying a widget aborts signal and disposes its isolated browser.",
