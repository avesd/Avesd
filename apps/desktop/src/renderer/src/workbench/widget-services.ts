import { createWidgetWorkspaceServices } from "../../../shared/widget-workspace";
import type { DesktopWidgetWorkspaceApi } from "../../../shared/widget-workspace";
...
export type WidgetServiceFactory = (
  ...
  lifetime?: { readonly signal: AbortSignal; readonly capabilities?: readonly WidgetWorkspaceCapability[] },
) => Pick<WidgetMountContext, "browser" | "configuration" | "data" | "catalog" | "navigation" | "management">;
...
export function createWidgetServices(dataSources: DataSourceService, browserControls?: BrowserControlsApi, workspace?: DesktopWidgetWorkspaceApi): WidgetServiceFactory {
  return (instance, updateConfiguration, lifetime) => {
    ...
    const scope = { workspaceId, dashboardId };
    return {
      ...(workspace && lifetime ? createWidgetWorkspaceServices(lifetime.capabilities ?? [], {
        invoke(request) {
          if (lifetime.signal.aborted) return Promise.reject(new Error("Widget was disposed."));
          return workspace.invoke(instanceId, request);
        },
        subscribe(listener) {
          if (lifetime.signal.aborted) return () => {};
          const unsubscribe = workspace.subscribe(() => { if (!lifetime.signal.aborted) listener(); });
          const dispose = () => { unsubscribe(); lifetime.signal.removeEventListener("abort", dispose); };
      ...
      data: {
        read: (inputId) => Promise.all((bindings[inputId] ?? []).map(async (id) => (await dataSources.read(scope, id)).value)),
        subscribe: (_inputId, listener) => dataSources.subscribe(listener),
        async update(inputId, value) {
          const ids = bindings[inputId] ?? [];
          if (ids.length !== 1 || !ids[0]) throw new Error(`widget input must have exactly one data source: ${inputId}`);
