...
export const MountedWidget = ({
  ...
  useEffect(() => {
    ...
    try {
      const controller = definition.mount(root, {
        ...widgetServices({ bindings: activeBindings, dashboardId, id: instanceId, pluginId, widgetTypeId, workspaceId },
          (configuration) => applyRef.current([{ configuration, id: instanceId, type: "configure" }]),
          { signal: abortController.signal, capabilities: definition.capabilities }),
        dashboardId,
        instanceId,
  ...
  }, [bindingsKey, dashboardId, definition, instanceId, pluginId, widgetServices, widgetTypeId, workspaceId]);
  ...
  }, [instance, definition, widgetServices]);
