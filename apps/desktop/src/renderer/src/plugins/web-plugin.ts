...
const styles = `
  ...
  * { box-sizing:border-box; }
  section { height:100%; display:flex; flex-direction:column; overflow:hidden;
    border:1px solid #cbd3c7; border-radius:12px; background:#f6f8f2; }
  header, .actions { display:flex; align-items:center; gap:6px; padding:8px; flex-wrap:wrap; }
  ...
  .viewport { flex:1; min-height:0; display:grid; place-content:center; padding:0;
  ...
  p { margin:6px 0; } .status { padding:0 8px 6px; overflow-wrap:anywhere; }
  pre { white-space:pre-wrap; overflow-wrap:anywhere; overflow:auto; margin:0; padding:12px; }
  [hidden] { display:none !important; }
`;

export function createWebPlugin(api: WebSurfaceApi, results: WebResults): PluginDefinition {
  const page: WidgetContribution = {
    widgetTypeId: "page", displayName: "Web page",
    description: "Embedded browser with temporary JS/CSS tools and a local-only result output.",
    configuration: { default: {}, version: 1 },
    sizing: { default: { width: 12, height: 20 },
      policy: { kind: "fixed", sizes: [{ width: 12, height: 20 }, { width: 24, height: 26 }] } },
    mount(root, context) {
      ...
      const show = (next: WebSurfaceState) => {
        if (disposed) return;
        if (state?.document !== next.document) consent.checked = false;
        state = next;
      ...
      const syncBounds = () => {
        ...
        const blocked = !!document.querySelector(".layout-editor, .agent-panel");
        ...
        const bounds = { x: rect.x, y: rect.y, width: rect.width, height: rect.height, visible };
        const key = JSON.stringify(bounds);
        if (key === lastBounds) return;
        lastBounds = key;
        void api.command({ type: "bounds", id: state.id, bounds }).catch(() => undefined);
      };
      const scheduleBounds = () => { if (!frame && !disposed) frame = requestAnimationFrame(syncBounds); };
      const refresh = async () => {
        if (!state) return;
        const version = ++refreshVersion;
        try {
          const next = await api.command({ type: "inspect", id: state.id });
          if (version === refreshVersion) { show(next); scheduleBounds(); }
        } catch { /* The instance may have been removed. */ }
      };
      const navigate = async (target: string) => {
        if (!state) return;
        consent.checked = false;
        try { show(await api.command({ type: "navigate", id: state.id, url: target })); scheduleBounds(); }
        catch { status.textContent = "Use HTTPS, or HTTP on localhost, without URL credentials."; }
      };
      section.querySelector(".go")!.addEventListener("click", () => void navigate(url.value));
      url.addEventListener("keydown", (event) => { if (event.key === "Enter") void navigate(url.value); });
      section.querySelector(".reload")!.addEventListener("click", () => void navigate(state?.url || url.value));
      ...
      consent.addEventListener("change", () => { if (state) show(state); });
      const revoke = () => { consent.checked = false; if (state) show(state); };
      code.addEventListener("input", revoke); mode.addEventListener("change", revoke);
      run.addEventListener("click", () => {
        if (!state || !consent.checked || running) return;
        running = true; run.disabled = true;
        const target = state;
        void api.command({ type: "run", id: target.id, document: target.document,
          origin: new URL(target.url).origin, code: code.value, mode: mode.value as "isolated" | "page" | "css" })
          .then((next) => { show(next); status.textContent = "Completed · temporary result available to bound widgets"; })
          .catch(() => { status.textContent = "Script failed or page changed. Return JSON; reload if the script is stuck."; })
          .finally(() => { running = false; consent.checked = false; run.disabled = true; });
      });
      section.querySelector(".clear")!.addEventListener("click", () => {
        if (state) void api.command({ type: "clear", id: state.id }).then(show);
      });
      ...
      return {
        ...
        dispose() {
          ...
          if (state) {
            ...
            void api.command({ type: "destroy", id: state.id }).catch(() => undefined);
          }
  ...
  const result: WidgetContribution = {
    widgetTypeId: "result", displayName: "Web result", description: "Displays a bound temporary web output. Never saved or sent to the agent.",
    configuration: { default: {}, version: 1 },
    inputs: [{ id: "result", displayName: "Web output", dataType: WEB_RESULT_TYPE }],
    sizing: { default: { width: 12, height: 10 }, policy: { kind: "fixed",
      sizes: [{ width: 12, height: 10 }, { width: 12, height: 20 }, { width: 24, height: 12 }] } },
    mount(root, context) {
      const style = document.createElement("style"); style.textContent = styles;
      const section = document.createElement("section");
      ...
      const refresh = async () => {
        ...
        try {
          ...
          if (current === version) output.textContent = value == null
            ? "Bind a Web output in Edit layout, then run a script in its Web page widget."
            : JSON.stringify(value, null, 2);
        } catch { if (current === version) output.textContent = "Web output unavailable. Reconnect a source in Edit layout."; }
      };
      const unsubscribe = context.data.subscribe("result", () => void refresh());
      return { update() { void refresh(); }, dispose() { ++version; void unsubscribe(); root.replaceChildren(); } };
    },
  };
  const controls: WidgetContribution = {
    widgetTypeId: "controls", displayName: "Browser controls",
    description: "Read, navigate, or click a browser explicitly bound by the dashboard host.",
    configuration: { default: {}, version: 1 },
    sizing: { default: { width: 12, height: 14 }, policy: { kind: "fixed",
      sizes: [{ width: 12, height: 14 }, { width: 12, height: 20 }] } },
    mount(root, context) {
      const style = document.createElement("style"); style.textContent = styles;
      const section = document.createElement("section");
      section.innerHTML = `<header><strong>Browser controls</strong></header>
        <div class="tools"><p>Bind a target and allowed actions in Edit layout → Browser control bindings.</p>
      ...
      const act = (action: "extract" | "navigate" | "click") => {
        if (!context.browser || busy) return;
        ...
        void operation.then((value) => {
          ...
          busy = false;
          if (!disposed) section.querySelectorAll("button").forEach((button) => { button.disabled = false; });
        });
      };
      section.querySelector(".read")!.addEventListener("click", () => act("extract"));
      section.querySelector(".navigate")!.addEventListener("click", () => act("navigate"));
      section.querySelector(".click")!.addEventListener("click", () => act("click"));
      return { update() {}, dispose() { disposed = true; root.replaceChildren(); } };
    },
  };
  return { id: WEB_PLUGIN_ID, apiVersion: 1, version: "0.1.0", activate(context) {
    context.effect(() => context.contributions.contribute(dashboardWidgetContribution, page));
    context.effect(() => context.contributions.contribute(dashboardWidgetContribution, result));
    context.effect(() => context.contributions.contribute(dashboardWidgetContribution, controls));
  } };
}
