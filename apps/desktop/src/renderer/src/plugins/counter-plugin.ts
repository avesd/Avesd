...
const counterWidget: WidgetContribution = {
  ...
  displayName: "Counter",
  inputs: [{ dataType: "avesd.number", displayName: "Count", id: "count", required: true }],
  mount(root, context) {
    ...
    const unsubscribe = context.data.subscribe("count", () => void renderValue());
  ...
  sizing: {
    default: { height: 7, width: 6 },
    policy: { kind: "fixed", sizes: [{ height: 7, width: 6 }, { height: 8, width: 8 }] },
  },
...
export const counterPlugin: PluginDefinition = {
  activate(context) {
    context.effect(() => context.contributions.contribute(dataSourceContribution, counterSource));
