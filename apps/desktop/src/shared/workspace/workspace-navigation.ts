...
export function parseWorkspaceNavigation(input: unknown): WorkspaceNavigationCommand {
    ...
    if (command.type === "renameWorkspace") {
        return {
            type: command.type,
            workspaceId: text(command.workspaceId) as WorkspaceId,
            name: text(command.name),
        };
    }
    ...
    if (!value) {
        throw new Error("Dashboard scope is required.");
    }
    const scope = {
        workspaceId: text(value.workspaceId) as WorkspaceId,
        dashboardId: text(value.dashboardId) as DashboardId,
    };
    if (command.type === "select" || command.type === "delete") {
        return {
            type: command.type,
            scope,
        };
