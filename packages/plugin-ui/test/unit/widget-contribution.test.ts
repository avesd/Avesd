/**
 * @author Avesd
 * @package Plugin UI
 * @namespace TestUnit
 * @description Widget Contribution Test
 */

import type { WidgetContribution } from "../../src/widget-contribution";
import { dashboardWidgetContribution } from "../../src/widget-contribution";
import { describe, expect, it, vi } from "vitest";

describe("widget UI contract", () => {

    it("supports a framework-neutral mount, update, and dispose lifecycle", () => {

        const update = vi.fn();
        const dispose = vi.fn();
        const widget: WidgetContribution = {
            configuration: {
                default: {},
                version: 1,
            },
            displayName: "Status",
            mount() {

                return {
                    dispose,
                    update,
                };
            },
            sizing: {
                default: {
                    height: 4,
                    width: 6,
                },
            },
            widgetTypeId: "status",
        };

        expect(dashboardWidgetContribution.id).toBe("dashboard.widget");
        expect(widget.configuration.version).toBe(1);
        expect(widget.sizing.default).toEqual({
            height: 4,
            width: 6,
        });
    });
});
