 ...
 * @namespace TestUnit
 * @description Persistent Workspace Repository Test
 */

import type { WorkspacePersistenceDriver } from "../../src/persistent-workspace-repository";
import { PersistentWorkspaceRepository } from "../../src/persistent-workspace-repository";
import type { WorkspaceId, WorkspaceSnapshot } from "../../src/workspace-model";
    ...
    it("serializes refresh with mutations and restores stored data after a rejected save", async () => {
        ...
        await Promise.all([
            repository.createWorkspace({
                id: "first" as WorkspaceId,
                name: "First",
            }),
            repository.refresh(),
            repository.createWorkspace({
                id: "second" as WorkspaceId,
                name: "Second",
            }),
        ]);
