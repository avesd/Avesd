 ...
 * @namespace TestUnit
 * @description In Memory Workspace Repository Test
 */

import { DashboardLayoutCoordinator } from "../../src/dashboard-layout";
...
const addWidget = async (
    ...
    const layouts = new DashboardLayoutCoordinator(
        ...
        (pluginId, widgetTypeId) =>
        {
            ...
            return pluginId === widgetDefinition.pluginId && widgetTypeId === widgetDefinition.widgetTypeId
                ? widgetDefinition
                : undefined;
        },
    );
...
describe("InMemoryWorkspaceRepository", () => {
    ...
    it("keeps dashboards, widgets, and data sources in explicit workspace scopes", async () => {
        ...
        await expect(repository.listWidgetInstances({
            dashboardId: ids.dashboardA,
            workspaceId: ids.workspaceA,
        })).resolves.toHaveLength(1);
        await expect(repository.listDataSources({ workspaceId: ids.workspaceA })).resolves.toHaveLength(1);
    });
