...
export const parseWorkspaceSnapshot = (input: unknown): WorkspaceSnapshot | undefined => {
  ...
  if (snapshot.version !== 1) throw new Error("Unsupported workspace data format");
  ...
  const requireWorkspace = (id: unknown) => {
    identifier(id);
    if (!workspaces.has(id)) invalid("workspace reference");
  };
  const requireDashboard = (workspaceId: unknown, dashboardId: unknown) => {
    requireWorkspace(workspaceId);
    identifier(dashboardId);
    if (dashboards.get(dashboardId)?.workspaceId !== workspaceId) invalid("dashboard ownership");
  };
  ...
  for (const source of sources.values()) {
    ...
    if (!Object.hasOwn(source, "value")) invalid("data source value");
    const scope = record(source.scope);
    requireWorkspace(scope.workspaceId);
    if (scope.kind === "dashboard") requireDashboard(scope.workspaceId, scope.dashboardId);
    else if (scope.kind !== "workspace") invalid("data source scope");
  }
  for (const widget of widgets.values()) {
    ...
    for (const [inputId, ids] of Object.entries(bindings)) {
      identifier(inputId);
      if (!Array.isArray(ids)) invalid("widget bindings");
      const seen = new Set<string>();
      for (const id of ids) {
        identifier(id);
        if (seen.has(id)) invalid("duplicate binding");
        seen.add(id);
  ...
  try {
    for (const dashboard of validated.dashboards) {
      assertDashboardLayout(validated.widgets.filter((widget) => widget.dashboardId === dashboard.id));
    }
  } catch {
...
function identifier(value: unknown): asserts value is string {
  text(value);
  if (!value.trim()) invalid("empty identifier");
}

function integer(value: unknown, minimum: number): void {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < minimum) invalid("invalid integer");
}
...
function entities(value: unknown): Map<string, Record<string, unknown>> {
  ...
  for (const item of value) {
    ...
    if (result.has(entity.id)) invalid("duplicate identifier");
...
function assertJson(value: unknown, ancestors: Set<object>): void {
  ...
  if (typeof value !== "object" || value === null) invalid("expected JSON data");
  if (ancestors.has(value)) invalid("cyclic JSON data");
  ancestors.add(value);
