...
it("queries another workspace without switching and returns only independent metadata projections", async () => {
  ...
  const second = await navigateWorkspace(first.snapshot, { type: "createWorkspace", name: "Second" }, createId);
  const dashboard = first.snapshot.dashboards[0]!;
  const snapshot = { ...second.snapshot, dashboards: second.snapshot.dashboards.map((item) => ({ ...item, viewState: { syntheticPrivate: "hidden" } })) };
  const result = readWorkspaceCatalog(snapshot, { type: "dashboards", workspaceId: first.state.scope.workspaceId });
  expect(result).toEqual([{ id: dashboard.id, workspaceId: dashboard.workspaceId, name: dashboard.name }]);
