...
export class InMemoryWorkspaceRepository implements WorkspaceRepository {
  ...
  async createDataSource(
    ...
    const created = { ...dataSource, revision: 0, scope };
  ...
  async listDataSources(scope: WorkspaceScope): Promise<readonly DataSource[]> {
    ...
    return [...this.#dataSources.values()].filter(
      (dataSource) => dataSource.scope.workspaceId === scope.workspaceId,
    );
  }
  ...
  async updateDataSource(
    ...
    const updated = { ...source, revision: source.revision + 1, value };
  ...
  async deleteDataSource(scope: WorkspaceScope, dataSourceId: DataSourceId): Promise<void> {
    ...
    for (const [id, widget] of this.#widgets) {
      const bindings = Object.fromEntries(Object.entries(widget.bindings).map(
        ([inputId, ids]) => [inputId, ids.filter((candidate) => candidate !== dataSourceId)],
      ));
      this.#widgets.set(id, { ...widget, bindings });
    }
    this.#emit();
  }
